...
public class CacheFileMap extends LruCacheMap<String, FileInfo> implements DisposableBean {
    ...
    private static class ShutdownHook {
        ...
        private static Collection<InternalReference> collection = new HashSet<>();
        ...
        private static void registerOne(CacheFileMap one) {
            collection.add(new InternalReference(one));
        }

        private static void unregisterOne(CacheFileMap one) {
            collection.remove(new InternalReference(one));
        }

        private static void doDestroy() {
            try {
                collection.parallelStream()
                        .map(InternalReference::get)
                        .filter(cacheFileMap -> cacheFileMap != null && !cacheFileMap.destroyed)
        ...
        private static class InternalReference extends WeakReference<CacheFileMap> {

            public InternalReference(CacheFileMap referent) {
                super(referent);
            }

            @Override
            public boolean equals(Object obj) {
                return (obj instanceof InternalReference)
                        && Objects.equals(this.get(), ((InternalReference) obj).get());
            }
