...
public class IntegrationConfiguration {
    ...
    public void init() {  // initialize the storage path builder
        storagePathBuilder.configure(storageAddress.isEmpty() ? null : storageAddress);
    }
    ...
    @Bean
    public CacheFileMap cacheFileMap(@Value("${files.cache.max-number:30}") int number) {
        return new CacheFileMap(number);
    }
